"""Stabilize CRM query performance for v5.4.2.

Revision ID: 0006_crm_stabilization
Revises: 0005_crm_enterprise
"""
...
def upgrade() -> None:
    op.create_index(
        "ix_crm_contacts_org_name",
        "crm_contacts",
        ["organization_id", "name"],
    )
    op.create_index(
        "ix_crm_interactions_org_occurred",
        "crm_interactions",
        ["organization_id", "occurred_at"],
    )
    op.create_index(
        "ix_crm_opportunities_org_stage_updated",
        "crm_opportunities",
        ["organization_id", "stage", "updated_at"],
    )
    op.create_index(
        "ix_crm_tasks_org_status_due",
        "crm_tasks",
        ["organization_id", "status", "due_at"],
    )
    op.create_index(
        "ix_crm_tasks_org_assignee_status",
        "crm_tasks",
