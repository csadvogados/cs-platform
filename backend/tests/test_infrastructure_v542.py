...
def test_docker_entrypoint_uses_runtime_port_and_runs_migrations():
    ...
    assert "0007_cs_captacao_mvp.py" in content
...
def test_required_migration_is_packaged():
    migration = BACKEND / "alembic" / "versions" / "0007_cs_captacao_mvp.py"
    assert migration.exists()
    content = migration.read_text(encoding="utf-8")
    assert 'revision = "0007_cs_captacao_mvp"' in content
    assert 'down_revision = "0006_crm_stabilization"' in content
